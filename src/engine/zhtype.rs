...
/// Result of Chinese variant detection.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ChineseType {
    Traditional,
    Simplified,
    Unknown,
}
...
impl ChineseType {
    ...
    pub fn name(self) -> &'static str {
        match self {
            ChineseType::Traditional => "traditional",
            ChineseType::Simplified => "simplified",
            ChineseType::Unknown => "unknown",
        }
    }
}

// Character tables for simplified/traditional detection.

static SIMPLIFIED_CHARS: LazyLock<FxHashSet<char>> = LazyLock::new(|| {
...
static TRADITIONAL_CHARS: LazyLock<FxHashSet<char>> = LazyLock::new(|| {
...
/// Detect whether the given text is Traditional Chinese, Simplified Chinese,
/// or indeterminate.
///
/// The detection works by counting characters that belong exclusively to the
/// simplified set vs. the traditional set. Simplified membership is checked
/// first (matching the original TypeScript behavior), so any character present
/// in both sets is counted as simplified only.
pub fn detect_chinese_type(text: &str) -> ChineseType {
